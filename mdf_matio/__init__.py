...
def get_mdf_parsers() -> List[str]:
    """Get the list of parsers defined for the MDF

    Returns:
        ([str]): Names of parsers that are compatible with the MDF
    """
    return noop_parsers + [name for name, info in get_available_adapters().items()
...
def generate_search_index(directory: str, validate_records=True) -> Iterable[dict]:
